...
def extract_coordinates_from_kmz(kmz_file_path, interval=100):
    ...
    for distance in range(0, int(geodesic((start_latitude, start_longitude), (end_latitude, end_longitude)).meters), interval):
        ...
        # interpolated_elevation = start_altitude + fraction * (end_altitude - start_altitude)
    ...
    return {
        ...
        'interpolated_elevations': interpolated_elevation
    }
...
for i, (coord, elevation) in enumerate(zip(result['interpolated_coordinates'], result['interpolated_elevations'])):
    print(f"Point {i + 1}: Latitude: {coord[0]}, Longitude: {coord[1]}, Elevation: {elevation}")
